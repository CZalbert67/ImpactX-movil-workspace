...
object WearableContract {
    ...
    fun isSupported(manufacturer: String?, model: String?, platform: String?): Boolean =
        manufacturer?.trim().equals(MANUFACTURER, ignoreCase = true) &&
            model?.trim().equals(MODEL, ignoreCase = true) &&
            platform?.trim().equals(PLATFORM, ignoreCase = true)
}
